		...
		@if (!isset($dataDiklat['searchNotFound']))
			@if ($dataDiklat->count() > 0)
				<form id="search-diklat" style='width: 70%'>
					<input type="text" name="search" id="search-input" value="{{ request()->has('search') ? request()->get('search') : '' }}" placeholder="Cari nama diklat atau pemilik diklat" class="form-control">
				</form>
			@endif
		@else
			<form id="search-diklat" style='width: 70%'>
